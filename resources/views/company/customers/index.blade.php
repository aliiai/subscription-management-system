<x-layouts.dashboard title="العملاء">
  <div class="flex h-full flex-col">
    ...
    <div data-animate="auth-card" class="flex shrink-0 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">أضف وأدر عملاء شركتك واربطهم بخطط الاشتراك.</p>
        </div>
    ...
    {{-- table card --}}
    <div data-animate="auth-card" data-customers
         data-url="{{ route('company.customers') }}"
        ...
        <div class="flex shrink-0 flex-col gap-3 border-b border-slate-100 px-4 py-4 md:flex-row md:items-center md:justify-between sm:px-6 dark:border-white/10">
            <div class="flex items-center gap-2">
                ...
                <span data-customers-loading class="hidden h-4 w-4 animate-spin rounded-full border-2 border-brand/30 border-t-brand"></span>
            </div>
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
                ...
                <div class="relative w-full sm:w-auto">
                    ...
                    <input type="search" data-customers-search value="{{ $filters['q'] ?? '' }}"
                           placeholder="ابحث بالاسم أو البريد أو الهاتف..."
                           class="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pe-3 ps-10 text-sm text-slate-900 outline-none transition focus:border-brand focus:bg-white focus:ring-2 focus:ring-brand/25 sm:w-64 dark:border-white/10 dark:bg-white/5 dark:text-slate-100">
                </div>

                {{-- plan filter --}}
                <div class="relative w-full sm:w-auto">
                    <select data-customers-plan
                        ...
                        <option value="none" @selected(($filters['plan'] ?? '') === 'none')>بدون خطة</option>
        ...
        <div data-customers-results class="flex min-h-0 flex-1 flex-col">
            @include('company.customers._results')
        </div>
    </div>
  </div>

    {{-- ===================== CREATE / EDIT MODAL ===================== --}}
    <div data-modal="customer" class="fixed inset-0 z-50 hidden">
        ...
        <div data-modal-dismiss class="absolute inset-0 flex items-center justify-center p-4">
            <div data-modal-panel class="w-full max-w-lg overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-slate-900">
                <div class="flex items-center gap-3 border-b border-slate-100 px-6 py-4 dark:border-white/10">
                    ...
                    <div class="flex-1">
                        <h3 class="text-base font-bold text-brand-navy dark:text-white" data-customer-modal-title>إضافة عميل جديد</h3>
                        <p class="text-xs text-slate-400">أدخل بيانات العميل ويمكنك ربطه بخطة اشتراك.</p>
                    </div>
                ...
                <form method="POST" action="{{ route('company.customers.store') }}" data-customer-form data-base="{{ url('company/customers') }}" class="grid grid-cols-2 gap-x-4 gap-y-3.5 px-6 py-5">
                    @csrf
                    ...
                    <div class="col-span-2">
                        <x-input-label for="customer_name" value="اسم العميل" />
                        <div class="relative">
                            <x-auth-icon name="user" />
                            <x-text-input id="customer_name" name="name" value="{{ old('name') }}" required placeholder="اسم العميل أو الجهة" class="ps-11!" />
                        </div>
                        <x-input-error :messages="$errors->get('name')" />
                    </div>

                    <div class="col-span-2 sm:col-span-1">
                        <x-input-label for="customer_email" value="البريد الإلكتروني (اختياري)" />
                        <div class="relative">
                            <x-auth-icon name="mail" />
                            <x-text-input id="customer_email" type="email" name="email" value="{{ old('email') }}" placeholder="customer@mail.com" class="ps-11!" dir="ltr" />
                        </div>
                        <x-input-error :messages="$errors->get('email')" />
                    </div>

                    <div class="col-span-2 sm:col-span-1">
                        <x-input-label for="customer_phone" value="رقم الهاتف (اختياري)" />
                        <div class="relative">
                            <x-auth-icon name="phone" />
                            <x-text-input id="customer_phone" name="phone" value="{{ old('phone') }}" placeholder="05xxxxxxxx" class="ps-11!" dir="ltr" />
                        </div>
                        <x-input-error :messages="$errors->get('phone')" />
                    </div>

                    <div class="col-span-2">
                        <x-input-label for="customer_plan" value="خطة الاشتراك (اختياري)" />
                        <div class="relative">
                            ...
                            <select id="customer_plan" name="plan_id"
                    ...
                    <div class="col-span-2 hidden" data-customer-start-wrapper>
                        <x-input-label for="customer_start" value="تاريخ بدء الاشتراك" />
                        <input id="customer_start" type="date" name="start_date" value="{{ old('start_date', now()->format('Y-m-d')) }}"
                    ...
                    <div class="col-span-2 mt-1 flex items-center justify-end gap-3 border-t border-slate-100 pt-4 dark:border-white/10">
                        ...
                        <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-l from-brand to-brand-blue px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand/25 transition hover:scale-[1.02]">
                            ...
                            <span data-customer-submit-text>حفظ العميل</span>
                        </button>
                    </div>
                </form>
            </div>
    ...
    {{-- ===================== DELETE MODAL ===================== --}}
    <div data-modal="customer-delete" class="fixed inset-0 z-50 hidden">
        <div data-modal-overlay class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm"></div>
        <div data-modal-dismiss class="absolute inset-0 flex items-center justify-center p-4">
            <div data-modal-panel class="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-2xl dark:border-white/10 dark:bg-slate-900">
                ...
                <h3 class="mt-4 text-lg font-bold text-brand-navy dark:text-white">حذف العميل</h3>
                <p class="mt-1.5 text-sm text-slate-500 dark:text-slate-400">
                    هل أنت متأكد من حذف العميل "<span class="font-semibold text-slate-700 dark:text-slate-200" data-delete-name></span>"؟ لا يمكن التراجع عن هذا الإجراء.
                </p>
    ...
    @elseif ($errors->any())
        <script>
            window.__openCustomerModal = { mode: 'create', id: '' };
        </script>
    @endif
</x-layouts.dashboard>
